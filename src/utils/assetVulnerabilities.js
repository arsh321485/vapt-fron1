...
/** Prefer any status-like field on register / fix rows */
export function rowStatusValue(row) {
  if (!row || typeof row !== 'object') return '';
  return (
    row.status ||
    row.vuln_status ||
...
export function closedVulnHostKey(vulnName, host) {
  return `${String(vulnName || '').trim().toLowerCase()}::${String(host || '').trim().toLowerCase()}`;
}

export function closedRecordVulnName(rec) {
  return String(rec?.vulnerability_name || rec?.plugin_name || rec?.vul_name || '').trim();
}

export function closedRecordHostName(rec) {
  return String(rec?.host_name || rec?.asset || rec?.host || rec?.hostname || '').trim();
}

/** Build Set of "vulnName::host" keys for closed fix records */
export function buildClosedVulnHostSet(closedRecords = []) {
  const set = new Set();
  (Array.isArray(closedRecords) ? closedRecords : []).forEach((rec) => {
    const status = rowStatusValue(rec);
    // Skip only when status is explicitly still open/active
    ...
    names.forEach((name) => set.add(closedVulnHostKey(name, host)));
  });
  return set;
}
...
export function isVulnClosedOnHost(vuln, closedFixVulns = [], host = '') {
  ...
  if (closedForHost.some((fix) => closedRecordMatchesVuln(fix, vuln))) return true;
  // Trust this vulnerability's own closed/resolved status (do not leave Closed in Active Threats).
...
/** Active Threats list only — closed / Fixed Recently items never belong here. */
export function isActiveThreatVulnerability(vuln, closedFixVulns = [], host = '') {
  if (!vuln) return false;
  if (isVulnClosedOnHost(vuln, closedFixVulns, host)) return false;
  return isActiveVulnStatus(vuln.status);
}

export function filterActiveThreatVulnerabilities(vulns, closedFixVulns = [], host = '') {
  return (Array.isArray(vulns) ? vulns : []).filter((v) =>
    isActiveThreatVulnerability(v, closedFixVulns, host),
  );
}
