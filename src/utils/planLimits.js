...
export function uniqueHostCountFromPayload(source) {
  ...
  return (
    Number(source.unique_ip_count) ||
    Number(source.unique_asset_count) ||
    Number(source.unique_host_count) ||
    Number(source.ip_count) ||
    Number(source.host_count) ||
    Number(source.hosts_count) ||
    Number(source.total_hosts) ||
    0
  );
}
...
export function detectedFileAssetCount(source, fallback = 0) {
  ...
  const unique = uniqueHostCountFromPayload(source);
  if (unique) return unique;
  const b = billingAssetBreakdown(source);
  return b.billable_asset_count || Number(fallback) || 0;
}

/**
 * File IP count for the recommended-plan screen.
 * Prefer unique IPs/hosts from the scan — do not Math.max with inflated original/locked totals.
 */
  ...
  const visit = (source) => {
    ...
    if (typeof source === "number") {
      if (Number.isFinite(source) && source > 0) {
        if (!uniqueBest) billedBest = Math.max(billedBest, source);
      }
      return;
    }
    if (typeof source === "string") {
      const n = Number(source);
      if (Number.isFinite(n) && n > 0 && !uniqueBest) billedBest = Math.max(billedBest, n);
      return;
    }
    ...
    const billed = billingAssetBreakdown(source).billable_asset_count;
    if (billed) billedBest = Math.max(billedBest, billed);
    if (source.results) visit(source.results);
    if (source.data && source.data !== source) visit(source.data);
  };
  sources.forEach(visit);
  return uniqueBest || billedBest;
}
