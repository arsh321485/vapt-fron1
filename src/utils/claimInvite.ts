...
const CLAIM_INVITE_STORED_AT_KEY = "vaptfix_claim_invite_stored_at";
...
function queryFromSearchParams(search: string): Record<string, unknown> {
...
export function readLiveMagicInvite(query: Record<string, unknown> = {}): string {
  if (typeof window !== "undefined") {
    try {
      const fromWindow = extractClaimInviteToken(queryFromSearchParams(window.location.search));
      if (fromWindow) return fromWindow;
      const hash = String(window.location.hash || "").replace(/^#\/?/, "");
      const hashSearch = hash.includes("?") ? hash.slice(hash.indexOf("?")) : hash;
      const fromHash = extractClaimInviteToken(queryFromSearchParams(hashSearch));
...
/**
 * Call immediately before send-otp / verify-otp / login.
 * Re-reads ?invite= from the address bar so a stale empty store cannot drop the claim.
 */
export function ensureClaimInviteCaptured(query: Record<string, unknown> = {}): string {
  const live = readLiveMagicInvite(query);
  if (live) storeClaimInviteToken(live);
  return currentClaimInviteToken(query);
}

/** Persist + return the token that must go on send-otp / verify-otp / login. */
export function currentClaimInviteToken(query: Record<string, unknown> = {}): string {
  const token = resolveSignupInviteToken(query);
  if (token) storeClaimInviteToken(token);
  return token;
}

export function attachInviteTokenToRequestData(data: unknown): unknown {
  // Always re-capture from the live URL right before the request leaves.
  const invite = ensureClaimInviteCaptured();
  if (!invite) {
    if (typeof window !== "undefined" && /[?&]invite=/.test(window.location.search || "")) {
      console.warn(
        "[magic-link] URL has ?invite= but invite_token could not be resolved for the request body",
      );
    }
    return data;
  }
  ...
  if (data && typeof data === "object" && !Array.isArray(data)) {
    return { ...(data as Record<string, unknown>), invite_token: invite };
  }
...
export function markClaimInviteSignup(email?: string) {
  ...
  try {
    sessionStorage.setItem("vaptfix_magic_link_unlimited", "1");
    localStorage.setItem("vaptfix_magic_link_unlimited", "1");
  } catch {
    /* ignore */
  }
}
