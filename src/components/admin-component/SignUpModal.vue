...
import { extractClaimInviteToken, isClaimInviteFlow, setClaimInviteValid, storeClaimInviteToken } from '@/utils/claimInvite';
  ...
  methods: {
    ...
    async handleAdminSignIn() {
      ...
      try {
        ...
        try {
          ...
          console.error('Admin post-login redirect error:', redirectErr);
        }
      } catch (err) {
    ...
    async checkAndRedirectAdmin() {
      ...
      try {
        ...
        if ((!loggedUser || !Object.keys(loggedUser).length) && raw && raw !== 'undefined' && raw !== 'null') {
      ...
      if (isClaimInviteFlow()) {
        ...
        if (sessionStorage.getItem('isNewUser') === 'true' && authStore.needsCommunicationStep()) {
        ...
        try {
          ...
          this.$router.replace(authStore.needsCommunicationStep() ? '/communication' : '/riskcriteria');
        }
        return;
      }
