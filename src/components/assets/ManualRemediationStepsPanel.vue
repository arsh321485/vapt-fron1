...
export default {
  ...
  methods: {
    ...
    clearFixPoll() {
      if (this._fixPollTimer) {
        clearTimeout(this._fixPollTimer);
        this._fixPollTimer = null;
      }
    },
    ...
    async initAdminFixVuln() {
      ...
      if (this.restoreCachedFixIfReady(knownFixId)) {
        ...
        } else if (this.fixNotStarted) {
          // Cached "not started" from an earlier visit — steps may be ready
          // by now, so recheck quietly instead of trusting the stale cache.
          this.loadAdminFixData(seq, { silent: true });
        }
        return;
      }

      await this.loadAdminFixData(seq, { silent: false });
    },
    async loadAdminFixData(seq, { silent }) {
      const knownFixId = this.fixId || this.fixVulnerabilityId || '';
      if (!silent) {
