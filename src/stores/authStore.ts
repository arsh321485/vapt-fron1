...
export const useAuthStore = defineStore("auth", {
  ...
  actions: {
    ...
    _automationPremiumFallback() {
      return "Automation scripts are not available on the Freemium plan — download counts will stay at 0 until you upgrade to Premium.";
    },
    ...
    initCompletedSteps() {
      ...
      const ids = [user?.id, user?.pk, user?.user_id, user?.email]
        .map((v) => String(v || "").trim())
        .filter(Boolean);
      const keys = [...ids.map((id) => `completedSteps_${id}`), "completedSteps"];
