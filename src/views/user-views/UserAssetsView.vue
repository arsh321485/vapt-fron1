                  ...
                  <template v-else>
                    ...
                    <div v-for="(asset, i) in pagedAssets" :key="asset.id || asset.asset || i"
                      ...
                      :class="{
                        ...
                        'asset-item-zero-vuln': isZeroVulnAsset(asset),
                      }"
                      ...
                      <div class="d-flex justify-content-between align-items-start asset-item-top">
                        ...
                        <span v-else-if="isZeroVulnAsset(asset)" class="sev-badge sev-zero">0 Vulns</span>
                      </div>
                      <p class="asset-sub">
                        <span
                          v-if="getAssetTypeBadge(asset)"
                      ...
                      <div class="d-flex gap-2 flex-wrap">
                        <span class="vuln-chip" :class="{ 'vuln-chip-zero': isZeroVulnAsset(asset) }">
              ...
              <div class="assets-right-panel">
                ...
                <div class="right-panel-scroll">
                  ...
                  <div v-if="activeTab === 'vulnerabilities'">
                    ...
                    <div v-else class="d-flex flex-column gap-3">
                      <div
                        ...
                        :class="{
                          'vuln-accordion-item--expanded': expandedVulnIndex === idx,
                          'vuln-accordion-item--manual': expandedVulnIndex === idx && currentVulnTab === 'manual',
                        }"
                        :ref="'vuln-' + idx"
                      >
                          ...
                          <div class="vuln-accordion-body">
                            ...
                            <div class="av-description-block">
                              ...
                              <div class="av-db-text-wrap">
                                <p
                                  class="av-db-text"
                                  :class="{ 'av-db-text--clamped': !isDescriptionExpanded(idx) }"
                                >{{ getDisplayDescription(vuln, idx) }}</p>
                                <button
                                  v-if="descriptionText(vuln).length > descriptionPreviewLimit"
                                  type="button"
                                  class="av-read-more"
                                  :class="{ 'av-read-more--inline': !isDescriptionExpanded(idx) }"
                              ...
                              >
                                ...
                                <div v-else class="av-manual-tab">
                                  <div class="av-asset-section">
                                    <ManualRemediationStepsPanel
                                      :is-user="true"
                                      :key="'closed-mf-' + String(closedItemAsVuln(item).plugin_id || closedItemAsVuln(item).fix_vulnerability_id || closedItemAsVuln(item).vul_name) + '-' + selectedAssetIp"
                                      :vuln-name="closedItemAsVuln(item).vul_name"
                                      :asset-ip="selectedAssetIp"
                                      :severity="closedItemAsVuln(item).severity"
                  ...
                  <div v-if="activeTab === 'exceptions'">
                    ...
                    <Teleport to="body">
                    <div class="modal fade sr-view-modal" id="userSupportRequestModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="false">
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content vc-modal-content">
        ...
        <div class="modal-body p-4">
          ...
          <p class="fw-semibold mb-3" style="font-size:14px;">{{ assetSrVulnName || '-' }}</p>
          <p class="vc-modal-section-label mb-1">Asset</p>
          <p class="fw-semibold mb-3" style="font-size:14px;">{{ activeIndex || selectedAssetIp || '-' }}</p>
          ...
          <div class="row g-2 mt-1">
            <div class="col-4" v-for="n in 6" :key="n">
              <span
                class="vc-step-pill"
                :class="[
                  isAssetSrStepCompleted(n) ? 'vc-step-pill-disabled' : '',
                  isAssetSrStepRaised(n) ? 'vc-step-pill-raised' : '',
                  assetSrStep === n && !isAssetSrStepRaised(n) && !isAssetSrStepCompleted(n) ? 'vc-step-pill-active' : '',
                  assetSrStep === n && isAssetSrStepRaised(n) ? 'vc-step-pill-raised-selected' : '',
                ]"
                :style="isAssetSrStepCompleted(n) ? 'cursor:not-allowed;opacity:0.5;' : ''"
                :title="isAssetSrStepCompleted(n) ? 'Step already completed' : (isAssetSrStepRaised(n) ? 'Support already raised   click to view' : 'Select this step')"
                @click="!isAssetSrStepCompleted(n) && selectAssetSrStep(n)"
              >Step {{ n }}</span>
            </div>
          </div>
...
class TLSConfigurator:
  ...
  computed: {
    ...
    filteredVulnerabilities() {
      ...
      if (this.statusFilter.includes('open')) {
        vulns = vulns.filter(v => matchesVulnStatusFilter(v, ['open']));
    ...
    filteredAssets() {
      ...
      const searched = this.searchQuery
        ? typed.filter(a => a.asset?.toLowerCase().includes(this.searchQuery.toLowerCase()))
        : typed;
      // Zero-vuln assets always sink to the bottom, below every asset that
      // has at least one finding — stable sort keeps everything else as-is.
      return [...searched].sort((a, b) => {
        const aZero = this.isZeroVulnAsset(a) ? 1 : 0;
        const bZero = this.isZeroVulnAsset(b) ? 1 : 0;
        return aZero - bZero;
      });
    },
    hasActiveAssetInTab() {
  ...
  methods: {
    ...
    closedItemAsVuln(item) {
      ...
      const name = item.plugin_name || item.vulnerability_name || item.vul_name || '';
    ...
    openAssetSupportModal() {
      const expanded = Number.isFinite(this.expandedVulnIndex)
        ? this.filteredVulnerabilities[this.expandedVulnIndex]
        : null;
      this.assetSrStep = null;
      this.assetSrVulnName = expanded?.vul_name || this.filteredVulnerabilities[0]?.vul_name || '';
    ...
    async loadClosedFixForAsset(assetIp) {
      const host = String(assetIp || this.activeIndex || '').trim();
      if (!host) {
        this.closedFixVulnerabilities = [];
        return;
      }
      this.loadingClosedFix = true;
      const res = await this.authStore.getUserClosedVulnerabilities(host, this.authStore.userSelectedTeam);
      ...
      if (res.status && res.data?.results) {
        ...
        this.closedFixVulnerabilities = res.data.results.filter((v) => {
          ...
          const h = String(v.host_name || v.asset || v.host || '').trim().toLowerCase();
          if (h && current && h !== current) return false;
          return true;
        });
      } else {
        this.closedFixVulnerabilities = [];
    ...
    async onManualFixSupportModal({ vulnName, step, completedSteps = [] } = {}) {
      ...
      this.assetSrJustSubmitted = false;
      this.assetSrRaisedSteps = [];
      this.assetSrCompletedSteps = (completedSteps || []).map(Number).filter((n) => Number.isFinite(n));
      this.assetSrRequestsByStep = {};
      this.assetSrFixVulnId = null;
      if (!this.assetSrVulnName) {
        Swal.fire('Error', 'Vulnerability context missing for support request', 'error');
        return;
      }
      ...
      if (this.assetSrStep && !this.isAssetSrStepCompleted(this.assetSrStep)) {
        this.selectAssetSrStep(this.assetSrStep);
      } else {
        this.assetSrStep = null;
      }
    },
    ...
    isAssetSrStepCompleted(step) {
      return this.assetSrCompletedSteps.includes(Number(step));
    },
    selectAssetSrStep(step) {
      const n = Number(step);
      if (this.isAssetSrStepCompleted(n)) return;
      this.assetSrStep = n;
      this.assetSrJustSubmitted = false;
    ...
    async loadAssetSrRequestsForVuln() {
      ...
      const res = await this.authStore.getUserSupportRequestsByHost(host, this.authStore.userSelectedTeam);
    ...
    async submitAssetSr() {
      ...
      if (this.isAssetSrStepCompleted(this.assetSrStep)) return;
      ...
      // Always bind to the currently selected asset in the left panel
      const asset = this.activeIndex || this.selectedAssetIp;
      if (!reportId || !asset) {
      ...
      if (!fixVulnId) {
        const selectedVuln = this.filteredVulnerabilities.find(v => v.vul_name === this.assetSrVulnName)
          || (this.authStore.selectedAssetVulnerabilities || []).find(v => v.vul_name === this.assetSrVulnName);
    ...
    // The host-scoped list doesn't reliably include the admin's replies, so
    // pull the full thread from the report-scoped endpoint (same resource
    // the reply POST uses) and merge it in — this is what makes admin
    // replies show up on the user's side.
    ...
    async liveRefreshPage() {
      ...
      if (inFixUi) return;
      if (this.activeIndex) {
        await this.authStore.fetchUserSingleAssetVulnerabilities(this.activeIndex, team);
      }
    },
    ...
    async loadHeldAssets() {
      const res = await this.authStore.fetchUserHeldAssets(true, this.authStore.userSelectedTeam);
      const held = res.status ? (res.assets || []) : [];
      this.hostAssetTypeMap = { ...loadHeldItemTypeMap(), ...this.hostAssetTypeMap };
      const prevByHost = {};
      (this.heldAssets || []).forEach((row) => {
        const host = String(row.asset || row.ip || row.host_name || '').trim().toLowerCase();
        if (host && row.asset_type) prevByHost[host] = row.asset_type;
      });
      this.heldAssets = held.map(a => {
    ...
    async setActive(asset) {
      ...
      this.expandedVulnIndex = null;
      this.expandedClosedIndex = null;
      await this.authStore.fetchUserSingleAssetVulnerabilities(asset.asset, this.authStore.userSelectedTeam);
      ...
      await this.loadClosedFixForAsset(asset.asset);
    },
    openFixPanelAlerts() {
      // First visit only   alerts self-dismiss after 5s and stay hidden afterwards.
    ...
    viewFixDetail(item) {
      ...
      const idx = this.closedFixVulnerabilities.findIndex(
        (v) =>
          ...
          String(v.plugin_name || v.vulnerability_name || '').toLowerCase() ===
            String(item.plugin_name || item.vulnerability_name || '').toLowerCase(),
      );
    ...
    /** True when an asset has no vulnerabilities at all (critical/high/medium/low all 0). */
    isZeroVulnAsset(asset) {
    ...
    async confirmDelete() {
      const selected = (this.filteredAssets || []).filter(a => a.selected);
      const reportId = this.authStore.userLatestReportId;
      if (!reportId) {
    ...
    async confirmHold() {
      ...
      const optimistic = selected.map((item) => ({
        ...
        held: true,
        selected: false,
        isInternal: item.member_type === 'internal' || item.isInternal,
      }));
      this.heldAssets = [
        ...optimistic,
...
/* Full viewport fill   header is position:fixed (out of flow); tabs already pad for it */
.assets-content {
  padding: 0;
  display: flex;
  flex-direction: column;
...
/* No open vulnerabilities at all — highlight so a clean asset stands out. */
.asset-item-zero-vuln {
  background: #eff6ff;
  border-left-color: #93c5fd !important;
}
.asset-item-zero-vuln:hover {
  background: #e0edff;
}
.asset-item-zero-vuln.asset-item-active {
  background: #f0fdf4;
}
...
.sev-zero     { background: #dbeafe !important; color: #2563eb !important; }
...
.ext-sev-low      { background: #d1fae5; color: #10b981; border: 1px solid #6ee7b7; }
