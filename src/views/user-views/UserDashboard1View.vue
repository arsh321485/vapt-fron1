        ...
        <div class="row">
          ...
          <div class="col-11 ud-content">
            ...
            <div class="row g-2 mb-3 align-items-stretch">
              ...
              <div class="modal fade" id="userInProcessModal" tabindex="-1" aria-hidden="true">
                ...
                <div class="col-md-3" v-for="team in allTeamsData" :key="team.key">
                  <div class="cv-team-card" :class="selectedTeam === team.key ? 'cv-team-card-active' : ''" @click="selectTeam(team.key)">
                    ...
                    <div class="cv-team-stats">
                      ...
                      <div v-if="team.medium != null" class="cv-stat-row">
                        <span class="cv-stat-dot cv-dot-medium"></span>
                        <span class="cv-stat-label">Medium</span>
                        <span class="cv-stat-val">{{ team.medium }}</span>
                      </div>
                      <div v-if="team.low != null" class="cv-stat-row">
                        <span class="cv-stat-dot cv-dot-low"></span>
                        <span class="cv-stat-label">Low</span>
                        <span class="cv-stat-val">{{ team.low }}</span>
                      </div>
                    </div>
                ...
                <div class="col-md-3" v-for="vuln in uniqueVulns.slice(0, 8)" :key="vuln.plugin_name">
                  <div class="cv-vuln-card">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                      <span class="cv-vuln-assets"><i class="bi bi-hdd-network me-1"></i>{{ getVulnAssetCount(vuln) }} affected asset{{ getVulnAssetCount(vuln) !== 1 ? 's' : '' }}</span>
...
export default {
  ...
  computed: {
    ...
    mteFilteredData() {
      ...
      const apiCritical = this.mteReportData.critical || [];
      const apiHigh     = this.mteReportData.high     || [];
      const apiMedium   = this.mteReportData.medium   || [];
      const apiLow      = this.mteReportData.low      || [];
    ...
    extOriginalDeadlineDisplay() {
      if (this.extPopupOriginalDeadlineDays !== null) {
        return `${this.extPopupOriginalDeadlineDays} Days`;
      }
      return this.extPopupSeverity ? (this.extOriginalDeadlines[this.extPopupSeverity] || '—') : '—';
    },
    ...
    allTeamsData() {
      const normalize = s => String(s || '').toLowerCase().replace(/\s+/g, ' ').trim();
      const canonicalTeam = (raw) => {
        const n = normalize(raw);
        if (!n) return '';
        if (n.includes('patch')) return 'patch management';
        if (n.includes('network')) return 'network security';
        if (n.includes('config')) return 'configuration management';
        if (n.includes('architect')) return 'architectural flaws';
        return n;
      };
      const teamConfigs = [
      ...
      const assignedSet = new Set((this.userTeams || []).map(canonicalTeam).filter(Boolean));
      const apiTeamSet = new Set(Object.keys(teams).map(canonicalTeam).filter(Boolean));
      const effectiveSet = assignedSet.size ? assignedSet : apiTeamSet;
      const visibleTeamConfigs = effectiveSet.size
        ? teamConfigs.filter(cfg => effectiveSet.has(canonicalTeam(cfg.key)))
        : [];

      return visibleTeamConfigs.map(cfg => {
  ...
  methods: {
    ...
    async refreshInProcessCount() {
      const store = useAuthStore();
      const result = await store.getUserDashboardRemediationInProcess();
      if (result.status) {
        this.inProcessCount = result.data?.total || 0;
        this.inProcessItems = result.data?.items || [];
        this.inProcessReportId = result.data?.report_id || this.authStore.userLatestReportId || null;
      } else {
        this.inProcessCount = 0;
        this.inProcessItems = [];
        this.inProcessReportId = this.authStore.userLatestReportId || null;
      }
    },
    ...
    async submitExtPopup() {
      ...
      if (res.status) {
        ...
        Swal.fire({
          ...
          customClass: { container: 'swal-above-drawer' },
          didOpen: () => {
            const container = document.querySelector('.swal-above-drawer');
            if (container) container.style.zIndex = '99999';
          },
        });
      }
    ...
    async loadMteExtensionData() {
      ...
      this.mteExtensionLoading = false;
      if (res.status && res.data?.teams) {
        this.mteApiTeams = res.data.teams;
        const teamMap = {};
...
.ext-header-close {
  ...
  display: flex; align-items: center; justify-content: center;
  font-size: 16px; cursor: pointer; transition: background 0.15s, border-color 0.15s; flex-shrink: 0;
}
...
.ext-submit-btn {
  background: #241447 !important; border-color: #241447 !important;
  border-radius: 999px !important; padding-left: 22px !important;
  padding-right: 22px !important; display: flex; align-items: center; gap: 6px;
}
.mte-modal-body {
  flex: 1 1 auto;
  min-height: 0;
...
.msu-modal-status-open { background: #fef2f2; color: #dc2626; }
.msu-modal-status-open .msu-modal-status-dot { background: #dc2626; }
...
.in-process-action-btn:hover { opacity: 0.9; }
.in-process-action-btn:active { transform: scale(0.97); }
.in-process-view-btn {
  background: #241447;
  color: #ffffff;
}
