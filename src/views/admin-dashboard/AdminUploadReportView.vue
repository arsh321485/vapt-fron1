...
export default {
  ...
  methods: {
    ...
    async promptUnpaidPlanChoice(source = 'upload', count = 0) {
      ...
      const n = Math.max(Number(count) || 0, this.recommendAssetCount());
      this.planSuggestResolved = false;
    ...
    async enforcePlanLimitAfterAssetsReady() {
      ...
      const fromFile = this.recommendAssetCount();
      // Parsed asset list is source of truth when it is not Freemium-capped at 5.
      const fullCount =
        uniqueIps > FREEMIUM_VISIBLE_ASSET_LIMIT
          ? uniqueIps
          : (fromFile || uniqueIps);
