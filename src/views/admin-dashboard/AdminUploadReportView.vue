          ...
          <div v-else-if="viewMode === 'upload'" class="aur-panel">
            ...
            <div class="aur-header">
              ...
              <p class="aur-subtitle">
                {{
                  activePlanFitNotice
                    ? `Upload a report with up to ${activePlanFitNotice.limit} IPs to continue on ${activePlanFitNotice.planName}.`
            ...
            <div v-if="activePlanFitNotice" class="aur-plan-fit-banner">
              <p>
                You chose <strong>{{ activePlanFitNotice.planName }}</strong>, which allows up to
                <strong>{{ activePlanFitNotice.limit }}</strong> IPs.
                The current report has <strong>{{ activePlanFitNotice.count }}</strong> IPs,
                so it cannot be used for this plan.
                {{ activePlanFitNotice.plan === 'freemium'
                  ? 'Please choose a new file that fits Freemium, then continue.'
                  : 'Please choose a new file that fits this plan, then complete payment.' }}
              </p>
            ...
            <div class="aur-actions">
              ...
              <button
                class="aur-btn-primary"
                :class="{ 'aur-btn-secondary': canContinueWithExistingFile && !selectedFiles.length }"
          ...
          <div v-else-if="viewMode === 'scope-csv'" class="aur-panel">
            ...
            <div class="aur-header">
              ...
              <p class="aur-subtitle">
                {{
                  activePlanFitNotice
                    ? `Upload a file with up to ${activePlanFitNotice.limit} targets to continue on ${activePlanFitNotice.planName}.`
          ...
          <div v-else-if="viewMode === 'scope-manual'" class="aur-panel">
            ...
            <div class="aur-header">
              ...
              <p class="aur-subtitle">
                {{
                  activePlanFitNotice
                    ? `Enter up to ${activePlanFitNotice.limit} targets to continue on ${activePlanFitNotice.planName}.`
...
export default {
  ...
  methods: {
    ...
    planContinueCopy(planId, source = 'upload') {
      ...
      if (plan === 'freemium') {
        return `${saved} Continue with Freemium — no payment needed. Next you will add users, set risk criteria, then open the dashboard.`;
      }
      if (plan === 'custom') {
    ...
    async continueWithSelectedPlan(planId, assetCount = 0) {
      ...
      if (!files.length) {
        ...
        this.uploadBeforePay = false;
        if (fromScope) markScopeAwaitingScan();
        this.goToPricing(planId, count, alreadyUploaded, fromScope ? { mode: 'testing' } : {});
        return;
      }
    ...
    isOnboardingScopeGate() {
      ...
      if (isClaimInviteFlow() || readClaimInviteToken()) return false;
      return true;
    },
