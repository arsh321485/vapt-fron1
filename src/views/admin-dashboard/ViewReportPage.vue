                  ...
                  <table class="simple-table">
                    ...
                    <tbody>
                      ...
                      <tr v-else v-for="row in filteredData" :key="row.id">
                        ...
                        <td>{{ row.found }}</td>
                        <td><span :class="['status-pill', (row.status || '').toLowerCase() === 'open' ? 'open' : 'closed']">{{ formatStatusLabel(row.status) }}</span></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
...
export default {
  ...
  computed: {
    ...
    riskScore() {
      if (this.apiRiskScore != null && this.apiRiskScore !== '') {
        const n = Number(this.apiRiskScore);
  ...
  methods: {
    ...
    async loadReportData(silent = false) {
      if (!silent) {
      ...
      try {
        ...
        if (d.executive_summary != null && String(d.executive_summary).trim()) {
          this.executiveSummary = String(d.executive_summary).trim();
        } else {
          this.executiveSummary = '';
        }
        if (d.risk_rating != null && String(d.risk_rating).trim()) {
          this.riskRating = String(d.risk_rating).trim();
        } else {
          this.riskRating = '';
        }
        if (d.risk_score != null && d.risk_score !== '') {
          this.apiRiskScore = d.risk_score;
        }
      ...
      const configs = [
        {
          ...
          data: {
            ...
            datasets: [{ label: 'Total Discovered', data: [critical, high, medium, low], backgroundColor: [...SEV_CHART], borderRadius: 6 }]
          },
          options: {
            responsive: true,
...
.stat-card.critical strong { color: #b42318; }
.stat-card.high strong { color: #dc2626; }
.stat-card.medium strong { color: #f59e0b; }
.stat-card.low strong { color: #10b981; }
.stat-card.critical { border-bottom: 3px solid #b42318; }
.stat-card.high { border-bottom: 3px solid #dc2626; }
.stat-card.medium { border-bottom: 3px solid #f59e0b; }
.stat-card.low { border-bottom: 3px solid #10b981; }
...
.sev-pill.high { background: #fee2e2; color: #dc2626; }
.sev-pill.medium { background: #fef3c7; color: #f59e0b; }
.sev-pill.low { background: #d1fae5; color: #10b981; }
