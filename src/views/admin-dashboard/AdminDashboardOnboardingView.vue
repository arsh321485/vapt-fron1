        ...
        <div class="row">
          ...
          <div class="col-11 pt-4 pb-5 pe-4 ps-4 flex-grow-1 position-relative" style="background: #f3f4f6; min-height: 100vh;">
            ...
            <div class="d-flex align-items-center justify-content-between mb-4 pt-2">
              <div class="d-flex align-items-center gap-2 position-relative" data-walkthrough-target="header">
                <h2 class="mb-0 fw-bold pt-5" style="font-size: 1.4rem; color: #1e293b;">Vulnerability Management Program</h2>
                <button
                  type="button"
                  class="dashboard-walkthrough-trigger mt-5"
            ...
            <div class="row g-2 mb-3 align-items-stretch">
              ...
              <!-- In-process vulnerabilities modal -->
              <div class="modal fade" id="inProcessModal" tabindex="-1" aria-hidden="true">
                ...
                >
                  ...
                  <div class="d-flex align-items-end justify-content-around gap-2 px-2" style="height: 82px; margin-top: 35px;">
                    ...
                    <div class="d-flex flex-column align-items-center gap-1" style="flex:1; max-width:52px;">
                      <span style="font-size:12px; font-weight:800; color:#b42318;">{{ authStore.vulnerabilities.critical || 0 }}</span>
                    ...
                    <!-- High -->
                    <div class="d-flex flex-column align-items-center gap-1" style="flex:1; max-width:52px;">
                      <span style="font-size:12px; font-weight:800; color:#dc2626;">{{ authStore.vulnerabilities.high || 0 }}</span>
              ...
              <!-- Card 3: Mitigation Timeline — same flow as header "Update Risk Criteria" -->
              <div class="col-4">
                <router-link
                  :to="{ path: '/riskcriteria', query: { returnTo: $route.fullPath } }"
                  class="text-decoration-none d-block h-100"
                  title="Update Risk Criteria"
                >
                <div
                  class="dash-card dash-card-tight dash-card-analytics row1-equal-card h-100 walkthrough-target dash-card-clickable"
                ...
                >
                  <div class="d-flex align-items-center gap-2 mb-2">
                    ...
                    <span class="dash-card-action-hint ms-auto">
                      ...
                      <div class="d-flex flex-column gap-2 flex-grow-1">
                        ...
                        <div>
                          ...
                          <div class="d-flex align-items-center gap-2">
                            <div style="height:5px;width:78%;background:#f8dede;border-radius:4px;overflow:hidden;">
            ...
            <section class="mb-5">
              ...
              <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="cv-more-details-btn" @click="openCommonVulnModal">
                              ...
                              <div v-for="(item, i) in overlayClosedVulns" :key="i" class="fixed-item">
                                <div class="d-flex align-items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p class="fixed-vuln-date">Status: {{ formatStatusLabel(item.status) }}</p>
                                  </div>
                                </div>
                              </div>
                            </div>
        ...
        <div class="mte-modal-box" @click.stop style="position:relative;">
          ...
          <div class="mte-severity-card mte-critical">
            <div class="mte-severity-head" @click="toggleMteSection('critical')">
              ...
              <div class="mte-head-right">
                <span v-if="mteFilteredData.critical.length === 0" class="mte-no-req-msg"><i class="bi bi-inbox"></i> No extension</span>
            ...
            <div v-if="mteOpenSections.critical" class="mte-table-wrap">
              <div v-if="mteFilteredData.critical.length === 0" class="mte-no-data">No extension for selected team.</div>
          ...
          <div class="mte-severity-card mte-high">
            <div class="mte-severity-head" @click="toggleMteSection('high')">
              ...
              <div class="mte-head-right">
                <span v-if="mteFilteredData.high.length === 0" class="mte-no-req-msg"><i class="bi bi-inbox"></i> No extension</span>
            ...
            <div v-if="mteOpenSections.high" class="mte-table-wrap">
              <div v-if="mteFilteredData.high.length === 0" class="mte-no-data">No extension for selected team.</div>
              <table v-else class="mte-table">
                <thead>
                  <tr>
                    <th>Asset (IP)</th>
          ...
          <div class="mte-severity-card mte-medium">
            <div class="mte-severity-head" @click="toggleMteSection('medium')">
              ...
              <div class="mte-head-right">
                <span v-if="mteFilteredData.medium.length === 0" class="mte-no-req-msg"><i class="bi bi-inbox"></i> No extension</span>
            ...
            <div v-if="mteOpenSections.medium" class="mte-table-wrap">
              <div v-if="mteFilteredData.medium.length === 0" class="mte-no-data">No extension for selected team.</div>
              <table v-else class="mte-table">
                <thead>
                  <tr>
                    <th>Asset (IP)</th>
          ...
          <div class="mte-severity-card mte-low">
            <div class="mte-severity-head" @click="toggleMteSection('low')">
              ...
              <div class="mte-head-right">
                <span v-if="mteFilteredData.low.length === 0" class="mte-no-req-msg"><i class="bi bi-inbox"></i> No extension</span>
            ...
            <div v-if="mteOpenSections.low" class="mte-table-wrap">
              <div v-if="mteFilteredData.low.length === 0" class="mte-no-data">No extension for selected team.</div>
              <table v-else class="mte-table">
                <thead>
                  <tr>
                    <th>Asset (IP)</th>
          ...
          <div style="flex:1; overflow-y:auto; padding:12px 0 8px;">
            ...
            <div v-if="cvModalGroupedVulns.critical.length > 0" class="mte-severity-card mte-critical">
              ...
              <div v-if="cvModalOpenSev === 'critical'" class="mte-table-wrap">
                <table class="mte-table">
                  ...
                  <tbody>
                    <template v-for="group in getCvVulnerabilityGroups('critical')" :key="'critical-' + group.name">
                      <tr class="cv-vuln-group-row" @click="toggleCvModalVuln('critical', group.name)">
                        <td colspan="6">
                          <div class="cv-vuln-group-head">
                            <span class="cv-vuln-group-title">{{ group.name }}</span>
                            <span class="cv-vuln-group-right">
                              <span class="mte-badge critical">{{ group.items.length }} {{ group.items.length === 1 ? 'Asset' : 'Assets' }}</span>
                              <i class="bi" :class="isCvModalVulnOpen('critical', group.name) ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
                            </span>
                          </div>
                        </td>
                      </tr>
                      <tr v-for="(vuln, vIdx) in group.items" :key="'critical-' + (vuln.id || vuln.host_name + vuln.plugin_name)" v-show="isCvModalVulnOpen('critical', group.name)">
                        <td style="color:#1e293b; font-size:12px; font-weight:600;">{{ vIdx + 1 }}</td>
            ...
            <div v-if="cvModalGroupedVulns.high.length > 0" class="mte-severity-card cv-high-card">
              ...
              <div v-if="cvModalOpenSev === 'high'" class="mte-table-wrap">
                <table class="mte-table">
                  <thead><tr><th style="width:40px;">S.No</th><th>Asset (IP)</th><th>OS</th><th>Vulnerability</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    <template v-for="group in getCvVulnerabilityGroups('high')" :key="'high-' + group.name">
                      <tr class="cv-vuln-group-row" @click="toggleCvModalVuln('high', group.name)">
                        <td colspan="6">
                          <div class="cv-vuln-group-head">
                            <span class="cv-vuln-group-title">{{ group.name }}</span>
                            <span class="cv-vuln-group-right">
                              <span class="mte-badge high">{{ group.items.length }} {{ group.items.length === 1 ? 'Asset' : 'Assets' }}</span>
                              <i class="bi" :class="isCvModalVulnOpen('high', group.name) ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
                            </span>
                          </div>
                        </td>
                      </tr>
                      <tr v-for="(vuln, vIdx) in group.items" :key="'high-' + (vuln.id || vuln.host_name + vuln.plugin_name)" v-show="isCvModalVulnOpen('high', group.name)">
                        <td style="color:#1e293b; font-size:12px; font-weight:600;">{{ vIdx + 1 }}</td>
                        <td style="font-weight:600; color:#1e293b;">{{ vuln.host_name || '—' }}</td>
                        <td style="color:#64748b; max-width:140px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" :title="vuln.os || '—'">{{ vuln.os || '—' }}</td>
                        <td style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#334155;" :title="vuln.plugin_name">{{ vuln.plugin_name }}</td>
            ...
            <div v-if="cvModalGroupedVulns.medium.length > 0" class="mte-severity-card cv-medium-card">
              ...
              <div v-if="cvModalOpenSev === 'medium'" class="mte-table-wrap">
                <table class="mte-table">
                  <thead><tr><th style="width:40px;">S.No</th><th>Asset (IP)</th><th>OS</th><th>Vulnerability</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    <template v-for="group in getCvVulnerabilityGroups('medium')" :key="'medium-' + group.name">
                      <tr class="cv-vuln-group-row" @click="toggleCvModalVuln('medium', group.name)">
                        <td colspan="6">
                          <div class="cv-vuln-group-head">
                            <span class="cv-vuln-group-title">{{ group.name }}</span>
                            <span class="cv-vuln-group-right">
                              <span class="mte-badge medium">{{ group.items.length }} {{ group.items.length === 1 ? 'Asset' : 'Assets' }}</span>
                              <i class="bi" :class="isCvModalVulnOpen('medium', group.name) ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
                            </span>
                          </div>
                        </td>
                      </tr>
                      <tr v-for="(vuln, vIdx) in group.items" :key="'medium-' + (vuln.id || vuln.host_name + vuln.plugin_name)" v-show="isCvModalVulnOpen('medium', group.name)">
                        <td style="color:#1e293b; font-size:12px; font-weight:600;">{{ vIdx + 1 }}</td>
                        <td style="font-weight:600; color:#1e293b;">{{ vuln.host_name || '—' }}</td>
                        <td style="color:#64748b; max-width:140px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" :title="vuln.os || '—'">{{ vuln.os || '—' }}</td>
                        <td style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#334155;" :title="vuln.plugin_name">{{ vuln.plugin_name }}</td>
            ...
            <div v-if="cvModalGroupedVulns.low.length > 0" class="mte-severity-card cv-low-card">
              ...
              <div v-if="cvModalOpenSev === 'low'" class="mte-table-wrap">
                <table class="mte-table">
                  <thead><tr><th style="width:40px;">S.No</th><th>Asset (IP)</th><th>OS</th><th>Vulnerability</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    <template v-for="group in getCvVulnerabilityGroups('low')" :key="'low-' + group.name">
                      <tr class="cv-vuln-group-row" @click="toggleCvModalVuln('low', group.name)">
                        <td colspan="6">
                          <div class="cv-vuln-group-head">
                            <span class="cv-vuln-group-title">{{ group.name }}</span>
                            <span class="cv-vuln-group-right">
                              <span class="mte-badge low">{{ group.items.length }} {{ group.items.length === 1 ? 'Asset' : 'Assets' }}</span>
                              <i class="bi" :class="isCvModalVulnOpen('low', group.name) ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
                            </span>
                          </div>
                        </td>
                      </tr>
                      <tr v-for="(vuln, vIdx) in group.items" :key="'low-' + (vuln.id || vuln.host_name + vuln.plugin_name)" v-show="isCvModalVulnOpen('low', group.name)">
                        <td style="color:#1e293b; font-size:12px; font-weight:600;">{{ vIdx + 1 }}</td>
                        <td style="font-weight:600; color:#1e293b;">{{ vuln.host_name || '—' }}</td>
                        <td style="color:#64748b; max-width:140px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" :title="vuln.os || '—'">{{ vuln.os || '—' }}</td>
                        <td style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#334155;" :title="vuln.plugin_name">{{ vuln.plugin_name }}</td>
...
export default {
  ...
  computed: {
    ...
    overlayFilteredVulns() {
      const list = filterActiveThreatVulnerabilities(
        this.authStore.selectedAssetVulnerabilities || [],
        this.overlayClosedVulns,
        this.overlayActiveIndex || this.authStore.selectedAssetDetail?.asset || '',
      );
  ...
  methods: {
    ...
    async refreshInProcessCount() {
      ...
      if (res.status) {
        this.inProcessCount = res.data?.total || 0;
        this.inProcessItems = res.data?.items || [];
      } else {
        this.inProcessCount = 0;
        this.inProcessItems = [];
      }
    },
    async openInProcessModal() {
      if (!this.inProcessCount) return;
      await this.$nextTick();
      const modalEl = document.getElementById('inProcessModal');
    ...
    toggleMteSection(section) { this.mteOpenSections[section] = !this.mteOpenSections[section]; },
    ...
    async openCommonVulnModal() {
      ...
      this.cvModalOpenVuln = { critical: "", high: "", medium: "", low: "" };
      ...
      if (mitResult.status) {
        ...
        this.setInitialCvModalVulnGroup('critical');
      }
    },
    ...
    toggleCvModalGroup(sev) {
      ...
      if (this.cvModalOpenSev) {
        this.setInitialCvModalVulnGroup(this.cvModalOpenSev);
      }
    },
    getCvVulnerabilityGroups(severity) {
      const rows = this.cvModalGroupedVulns?.[severity] || [];
      const grouped = new Map();
      rows.forEach((row) => {
        const name = String(row.plugin_name || "Unnamed Vulnerability").trim() || "Unnamed Vulnerability";
        if (!grouped.has(name)) grouped.set(name, []);
        grouped.get(name).push(row);
      });
      return Array.from(grouped.entries()).map(([name, items]) => ({ name, items }));
    },
    setInitialCvModalVulnGroup(severity) {
      if (this.cvModalOpenVuln[severity]) return;
      const groups = this.getCvVulnerabilityGroups(severity);
      this.cvModalOpenVuln[severity] = groups.length ? groups[0].name : "";
    },
    toggleCvModalVuln(severity, vulnName) {
      this.cvModalOpenVuln[severity] = this.cvModalOpenVuln[severity] === vulnName ? "" : vulnName;
    },
    isCvModalVulnOpen(severity, vulnName) {
      return this.cvModalOpenVuln[severity] === vulnName;
    },
    ...
    getMitigationLabel(sev) {
      // Prefer saved risk criteria — summary mitigation_timeline can stay stale after update.
      const criteria = this.riskCriteria?.[sev];
      if (criteria != null && String(criteria).trim()) {
        return String(criteria).trim();
      }
    ...
    */
    ...
    async loadRiskCriteria() {
      ...
      if (res.status && res.data) {
        const latest =
          res.data.critical !== undefined || res.data.high !== undefined
            ? res.data
            : (Array.isArray(res.data.risk_criteria) && res.data.risk_criteria[0]) || null;
        if (latest) {
          this.riskCriteria = {
          ...
          // Keep gauge store aligned so summary payload cannot flash old SLA labels.
          this.authStore.applyRiskCriteriaToMitigationGauges(this.riskCriteria);
        }
      }
    },
    ...
    loadDashboardData() {
      ...
      this._dashboardLoadInFlight = Promise.allSettled([
        ...
        .then((results) => {
          if (this.dashboardLoadToken !== token) return;
          // Summary may return stale mitigation_timeline — re-assert latest risk criteria on gauges.
          if (this.riskCriteria?.critical || this.riskCriteria?.high || this.riskCriteria?.medium || this.riskCriteria?.low) {
            this.authStore.applyRiskCriteriaToMitigationGauges(this.riskCriteria);
          }
          ...
          if (summaryResult.status === "fulfilled" && summaryResult.value?.status) {
            ...
            // Only update if new value is valid (prevent flicker to 0 on reload)
            if (support.total !== undefined) this.supportTotal = support.total;
            if (support.pending !== undefined) this.supportPending = support.pending;
            if (support.closed !== undefined) this.supportClosed = support.closed;

            if (fixed.total_fixed !== undefined) this.vulFixedTotal = fixed.total_fixed;
            if (fixed.critical_fixed !== undefined) this.vulFixedCritical = fixed.critical_fixed;
            if (fixed.high_fixed !== undefined) this.vulFixedHigh = fixed.high_fixed;
...
.dash-card-clickable:hover {
  box-shadow: 0 4px 14px rgba(15, 105, 110, 0.12);
  border-color: #c5e4e6;
}
.dash-card-action-hint {
  font-size: 11px;
  font-weight: 700;
  color: #0f696e;
  white-space: nowrap;
...
.dash-card-clickable:hover .dash-card-action-hint {
  opacity: 1;
...
.mte-status-rejected { background: #f8dede; color: #b42318; }
.mte-no-data {
  padding: 18px 20px;
  font-size: 13px;
  color: #94a3b8;
...
.mte-critical { border-color: #fca5a5; background: #f8dede; box-shadow: inset 0 0 0 1px #fca5a5; }
.mte-high { border-color: #fdba74 !important; background: #fed7aa !important; box-shadow: inset 0 0 0 1px #fdba74; }
.mte-medium { border-color: #fcd34d !important; background: #fef3c7 !important; box-shadow: inset 0 0 0 1px #fcd34d; }
.mte-low { border-color: #86efac !important; background: #d1fae5 !important; box-shadow: inset 0 0 0 1px #86efac; }
.mte-severity-head {
  padding: 16px 20px;
  display: flex;
  align-items: center;
...
.mte-high-icon { color: rgb(180, 35, 24); }
.mte-medium-icon { color: rgb(245, 158, 11); }
.mte-low-icon { color: rgb(16, 185, 129); }
...
.bg-maroon { background-color: #b42318 !important; }
...
.in-process-action-btn:hover {
  opacity: 0.9;
}
.in-process-action-btn:active {
