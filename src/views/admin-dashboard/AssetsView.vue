                          ...
                          <div class="modal-body p-4">
                            ...
                            <div class="d-flex flex-wrap gap-2 mb-4">
                              <template v-if="supportRequestStepPills(selectedSupportRequest).length">
                                <span v-for="(pill, i) in supportRequestStepPills(selectedSupportRequest)" :key="i" class="sr-step-pill">
                                  {{ pill }}
                                </span>
                              </template>
                              <template v-else>
  ...
  methods: {
    ...
    async loadAutomationScripts() {
      ...
      if (reportId) {
        ...
        // resolveAutomationMatched() above degrades to the legacy system
        // while this is still in flight, so nothing needs to await it.
        void this.authStore.fetchVulnerabilityCardsByReport(reportId);
      }
    ...
    async liveRefreshPage() {
      ...
      // from the user never appeared while both had the same thread open
     ...
     * step under different keys (step_number as a plain number, or step as
     * "all"/a comma list) — and the admin list-by-host endpoint may not echo
     * the same field name back. Check every plausible field so a genuinely
    ...
    getFilteredSortedAssets() {
      ...
      return list.sort((a, b) => {
        ...
        // has at least one finding — regardless of severity rank.
