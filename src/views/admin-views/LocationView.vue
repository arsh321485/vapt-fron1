<template>
  <main class="loc-main">
    ...
    <div v-show="!gateChecking && !redirecting" class="loc-page">
...
import { isScopeAwaitingScan, isScopeFileAwaitingSuperadmin, readStoredAdminEmail } from "@/utils/scopeScanGate";
...
import { consumeFreemiumActiveNotice, isActiveSubscription, isFreemiumPlan, isFreemiumTeamLimitMessage, isMagicLinkUnlimited } from "@/utils/planLimits";
import {
...
export default {
  ...
  //   },
    ...
    async loadAddedUsers(force = false) {
      const res = await this.authStore.fetchUsersByAdmin(force);
    ...
    async maybeShowFreemiumNotice() {
      try {
        ...
        // Magic-link unlimited: never show Freemium banner / upgrade UI.
        this.showFreemiumBanner = !!(
          !data?.magic_link_unlimited &&
      ...
      // Magic-link unlimited must never show Freemium toast / banner.
    ...
    async liveRefreshPage() {
      await this.loadAddedUsers(true);
    },
  },
  async mounted() {
    dismissUploadReportModal();
    ...
    try {
      if (!isClaimInviteFlow()) {
        ...
        if (!status?.hasReport) {
          ...
          // Scope submitted but scan not ready → wait page; otherwise force upload first.
          if (
            isScopeAwaitingScan() ||
            isScopeFileAwaitingSuperadmin(readStoredAdminEmail()) ||
            (await this.authStore.hasSubmittedScope())
          ) {
            await this.$router.replace("/waiting-for-report");
          } else {
