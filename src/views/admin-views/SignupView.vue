...
export default {
  ...
  methods: {
    async handleSignup() {
      ...
      try {
        ...
        const result = await authStore.signupSendOtp({
          ...
          invite_token: currentClaimInviteToken(this.$route?.query || {}) || this.inviteToken,
        })
        ...
        if (result.status) {
          ...
          const token = String(result.invite_token || resolveSignupInviteToken(this.$route?.query || {}) || this.inviteToken || '').trim()
          if (token) storeClaimInviteToken(token)
          this.$router.push({ path: '/signin', query: token ? { invite: token } : {} })
        } else {
    ...
    async handleVerifyOtp() {
      ...
      try {
        const authStore = useAuthStore()
        const inviteToken = currentClaimInviteToken(this.$route?.query || {}) || this.inviteToken
